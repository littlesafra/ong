...
for(i in 1:nrow(data)){
  print(i, nrow(data))
  data[i,1] <- chartr('áéíóúñ','aeioun', data[i,1])
  print(data[i,1])
}

#Insertar comarcas
for(i in 1:nrow(data)){
  myQuery <- paste0("INSERT INTO destino (poblacion) VALUES ('", data[i,1],"')")
...
#AsignaciÃ³n de paquetes a cada comarca
library(dplyr)
...
for(col in 1:5){
  for(row in 1:20){
    cluster <- Necesidades_Destino[row, 7]
       ...
       print(necesidadDestino<-as.numeric(Necesidades_Destino[row, col+1]))
       
       necesidadTotal<-as.numeric(total.necesidad.cluster[cluster, col])
       print(necesidadTotal<-as.numeric(total.necesidad.cluster[cluster, col]))

       recursoTotal<-as.numeric(total.recursos.cluster[cluster,col])
       print(recursoTotal<-as.numeric(total.recursos.cluster[cluster,col]))

...
library(DBI)
con <- dbConnect(RMySQL::MySQL(), user="root", host="localhost", password="", dbname="database_ong")
  ...
  for(row in 1:20){
    ...
    cluster <- Necesidades_Destino[row, 7]
       ...
       if(is.na(maximo$`max(id)`)){
          ...
          myQuery <- paste0("select stock from producto where productoID = '",col,"';")
          stock <- dbGetQuery(con,myQuery)
          stock <- stock - eragiketa
          myQuery <- paste0("update producto set stock = '",stock,"' where productoID = '",col,"';")
          dbSendQuery(con,myQuery)
